...
# batchExtractSignal.sh [pairFile] [peakDir] [signalDir] [oDir] [format]
...
while read i
  ...
  echo "extractSignal -i=${peakFile} -t=${sigFile} -us=peak -sl=1250 -sr=1250 -fw=true -o=${outFile} -of=${FORMAT} -ov=signal -mf=samplerate -mp=10" >> ${submitScript} # 2500 bp windows with 10 bp sampling
  if [[ ${FORMAT} == 'cagt' ]]
      then
